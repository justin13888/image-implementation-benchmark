...
public:
    ...
    std::vector<uint8_t> run(const Args& args) override {
        ...
        while (cinfo.next_scanline < cinfo.image_height) {
            uint8_t* row_pointer = const_cast<uint8_t*>(input_data.data() + cinfo.next_scanline * row_stride);
    ...
    void verify(const Args& args, const std::vector<uint8_t>& output) override {
        ...
        if (output.size() < 2 || output[0] != 0xFF || output[1] != 0xD8) {
            throw std::runtime_error("Output is not a valid JPEG (missing SOI marker)");
        }
    }
