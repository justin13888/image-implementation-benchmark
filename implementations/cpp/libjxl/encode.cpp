...
public:
    ...
    std::vector<uint8_t> run(const Args& args) override {
        auto runner = JxlThreadParallelRunnerMake(nullptr, args.threads > 0 ? args.threads : JxlThreadParallelRunnerDefaultNumWorkerThreads());
        ...
        if (JXL_ENC_SUCCESS != JxlEncoderAddImageFrame(frame_settings, &pixel_format, const_cast<uint8_t*>(input_data.data()), input_data.size())) {
        ...
        while (process_result == JXL_ENC_NEED_MORE_OUTPUT) {
            ...
            compressed.insert(compressed.end(), chunk.data(), chunk.data() + bytes_written);
            next_out = chunk.data();
